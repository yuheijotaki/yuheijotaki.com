...
- パッケージマネージャ: **npm**
- **Renovate 併用**: routine な patch+minor は Renovate（`renovate.json`、毎週月曜に automerge）が処理する。このスキルは「今すぐ手動で一括更新したい」「major を対話で選別したい」ときに使う
...
> webServer 設定（`playwright.config.ts`）が `npm run serve`（build + `.vercel/output/static` の静的サーブ）を自動実行するので手動起動は不要。ただしポート 4321 に dev server が既に立っている場合はそれを再利用する（`reuseExistingServer: true`）。本番相当でテストしたいなら dev server を止めてから実行すること。
...
1. コミットメッセージをドラフトしてユーザーに提示（**承認を得てからコミット**）
   ...
   - 末尾には `Co-Authored-By: Claude <noreply@anthropic.com>` を含める
...
| dev server が残った                    | `pkill -f "astro dev"` または TaskStop でタスク ID 指定で停止 |
