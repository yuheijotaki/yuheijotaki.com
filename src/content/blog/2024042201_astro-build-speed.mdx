---
title: 各ホスティングサービスでAstro（SSG）のビルド時間を測ってみる
description: Cloudflare, Vercel, Netlify, AWS Amplify（Hosting） にてAstro（SSG）のビルド時間を計測してみたメモです。
pubDate: 2024-04-22T00:00:00.000Z
---

## 概要

Cloudflare, Vercel, Netlify, AWS Amplify（Hosting） にてAstro（SSG）のビルド時間を計測してみた。
...
構築イメージ(Amazon Linux:2023)以外を選択、Node.jsのバージョン変更などをしてもビルド時間は差がない。
